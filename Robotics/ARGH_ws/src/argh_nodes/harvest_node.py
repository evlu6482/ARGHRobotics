...
def harvest_ang(tomato_height):
	...
	global total_height
	global bench_height
	ang_dif = ( tomato_height/(total_height - bench_height) ) * (max_angle - min_angle) 
...
def harvest_tomatos(data):
	...
	if((data.position.x <= 1) and (data.position.y <= 1) and (data.position.z <= 1) ):
		...
		#open gripper
		if(data.position.z >= total_height):
			...
			wpose.position.y = -0.300
			waypoints.append(copy.deepcopy(wpose))

			(plan, fraction) = group.compute_cartesian_path(waypoints,0.01,0.01)
			time.sleep(2.4)
			

		elif( (data.position.z < total_height ) and (data.position.z > 0.05)):
